        ...
        <div class="row">
            <div class="col-12">
                <div class="card-grid-2-image-left">
                    <div class="image-box"><img src="{{ RvMedia::getImageUrl($company->logo) }}"
                                                alt="{{ $company->name }}"></div>
                    <div class="right-info"><a class="name-job"
                                               href="{{ $company->url }}">{{ $company->name }}</a><span
                            class="location-small">{{ $company->state_name }}, {{ $company->country_name }}</span></div>
                </div>
            </div>
        </div>
        <div class="card-block-info">
            <h4><a href="{{ $company->url }}">{{ $company->name }}</a></h4>
